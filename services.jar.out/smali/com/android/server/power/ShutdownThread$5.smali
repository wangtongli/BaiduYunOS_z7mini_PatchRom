.class Lcom/android/server/power/ShutdownThread$5;
.super Ljava/lang/Thread;
.source "ShutdownThread.java"


# annotations
.annotation system Ldalvik/annotation/EnclosingMethod;
    value = Lcom/android/server/power/ShutdownThread;->shutdownRadios(I)V
.end annotation

.annotation system Ldalvik/annotation/InnerClass;
    accessFlags = 0x0
    name = null
.end annotation


# instance fields
.field final synthetic this$0:Lcom/android/server/power/ShutdownThread;

.field final synthetic val$done:[Z

.field final synthetic val$endTime:J

.field final synthetic val$modemEndTime:J


# direct methods
.method constructor <init>(Lcom/android/server/power/ShutdownThread;JJ[Z)V
    .locals 0
    .parameter
    .parameter
    .parameter
    .parameter

    .prologue
    .line 417
    iput-object p1, p0, Lcom/android/server/power/ShutdownThread$5;->this$0:Lcom/android/server/power/ShutdownThread;

    iput-wide p2, p0, Lcom/android/server/power/ShutdownThread$5;->val$endTime:J

    iput-wide p4, p0, Lcom/android/server/power/ShutdownThread$5;->val$modemEndTime:J

    iput-object p6, p0, Lcom/android/server/power/ShutdownThread$5;->val$done:[Z

    invoke-direct {p0}, Ljava/lang/Thread;-><init>()V

    return-void
.end method


# virtual methods
.method public run()V
    .locals 15

    .prologue
    .line 423
    const-string v11, "nfc"

    invoke-static {v11}, Landroid/os/ServiceManager;->checkService(Ljava/lang/String;)Landroid/os/IBinder;

    move-result-object v11

    invoke-static {v11}, Landroid/nfc/INfcAdapter$Stub;->asInterface(Landroid/os/IBinder;)Landroid/nfc/INfcAdapter;

    move-result-object v6

    .line 425
    .local v6, nfc:Landroid/nfc/INfcAdapter;
    const-string v11, "phone"

    invoke-static {v11}, Landroid/os/ServiceManager;->checkService(Ljava/lang/String;)Landroid/os/IBinder;

    move-result-object v11

    invoke-static {v11}, Lcom/android/internal/telephony/ITelephony$Stub;->asInterface(Landroid/os/IBinder;)Lcom/android/internal/telephony/ITelephony;

    move-result-object v8

    .line 427
    .local v8, phone:Lcom/android/internal/telephony/ITelephony;
    const-string v11, "bluetooth_manager"

    invoke-static {v11}, Landroid/os/ServiceManager;->checkService(Ljava/lang/String;)Landroid/os/IBinder;

    move-result-object v11

    invoke-static {v11}, Landroid/bluetooth/IBluetoothManager$Stub;->asInterface(Landroid/os/IBinder;)Landroid/bluetooth/IBluetoothManager;

    move-result-object v0

    .line 432
    .local v0, bluetooth:Landroid/bluetooth/IBluetoothManager;
    if-eqz v6, :cond_0

    :try_start_0
    invoke-interface {v6}, Landroid/nfc/INfcAdapter;->getState()I

    move-result v11

    const/4 v12, 0x1

    if-ne v11, v12, :cond_5

    :cond_0
    const/4 v7, 0x1

    .line 434
    .local v7, nfcOff:Z
    :goto_0
    if-nez v7, :cond_1

    .line 435
    const-string v11, "ShutdownThread"

    const-string v12, "Turning off NFC..."

    invoke-static {v11, v12}, Landroid/util/Log;->w(Ljava/lang/String;Ljava/lang/String;)I

    .line 436
    const/4 v11, 0x0

    invoke-interface {v6, v11}, Landroid/nfc/INfcAdapter;->disable(Z)Z
    :try_end_0
    .catch Landroid/os/RemoteException; {:try_start_0 .. :try_end_0} :catch_0

    .line 444
    :cond_1
    :goto_1
    if-eqz v0, :cond_2

    :try_start_1
    invoke-interface {v0}, Landroid/bluetooth/IBluetoothManager;->isEnabled()Z

    move-result v11

    if-nez v11, :cond_6

    :cond_2
    const/4 v1, 0x1

    .line 445
    .local v1, bluetoothOff:Z
    :goto_2
    if-nez v1, :cond_3

    .line 446
    const-string v11, "ShutdownThread"

    const-string v12, "Disabling Bluetooth..."

    invoke-static {v11, v12}, Landroid/util/Log;->w(Ljava/lang/String;Ljava/lang/String;)I

    .line 447
    const/4 v11, 0x0

    invoke-interface {v0, v11}, Landroid/bluetooth/IBluetoothManager;->disable(Z)Z
    :try_end_1
    .catch Landroid/os/RemoteException; {:try_start_1 .. :try_end_1} :catch_1

    .line 455
    :cond_3
    :goto_3
    const/4 v9, 0x1

    .line 456
    .local v9, radioOff:Z
    :try_start_2
    invoke-static {}, Landroid/telephony/MSimTelephonyManager;->getDefault()Landroid/telephony/MSimTelephonyManager;

    move-result-object v11

    invoke-virtual {v11}, Landroid/telephony/MSimTelephonyManager;->isMultiSimEnabled()Z

    move-result v11

    if-eqz v11, :cond_8

    .line 457
    const-string v11, "phone_msim"

    invoke-static {v11}, Landroid/os/ServiceManager;->checkService(Ljava/lang/String;)Landroid/os/IBinder;

    move-result-object v11

    invoke-static {v11}, Lcom/android/internal/telephony/msim/ITelephonyMSim$Stub;->asInterface(Landroid/os/IBinder;)Lcom/android/internal/telephony/msim/ITelephonyMSim;

    move-result-object v5

    .line 459
    .local v5, mphone:Lcom/android/internal/telephony/msim/ITelephonyMSim;
    if-eqz v5, :cond_a

    .line 462
    const/4 v3, 0x0

    .local v3, i:I
    :goto_4
    invoke-static {}, Landroid/telephony/MSimTelephonyManager;->getDefault()Landroid/telephony/MSimTelephonyManager;

    move-result-object v11

    invoke-virtual {v11}, Landroid/telephony/MSimTelephonyManager;->getPhoneCount()I

    move-result v11

    if-ge v3, v11, :cond_a

    .line 464
    if-eqz v9, :cond_7

    invoke-interface {v5, v3}, Lcom/android/internal/telephony/msim/ITelephonyMSim;->isRadioOn(I)Z

    move-result v11

    if-nez v11, :cond_7

    const/4 v9, 0x1

    .line 465
    :goto_5
    invoke-interface {v5, v3}, Lcom/android/internal/telephony/msim/ITelephonyMSim;->isRadioOn(I)Z

    move-result v11

    if-eqz v11, :cond_4

    .line 466
    const-string v11, "ShutdownThread"

    new-instance v12, Ljava/lang/StringBuilder;

    invoke-direct {v12}, Ljava/lang/StringBuilder;-><init>()V

    const-string v13, "Turning off radio on Subscription :"

    invoke-virtual {v12, v13}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v12

    invoke-virtual {v12, v3}, Ljava/lang/StringBuilder;->append(I)Ljava/lang/StringBuilder;

    move-result-object v12

    invoke-virtual {v12}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v12

    invoke-static {v11, v12}, Landroid/util/Log;->w(Ljava/lang/String;Ljava/lang/String;)I

    .line 467
    const/4 v11, 0x0

    invoke-interface {v5, v11, v3}, Lcom/android/internal/telephony/msim/ITelephonyMSim;->setRadio(ZI)Z
    :try_end_2
    .catch Landroid/os/RemoteException; {:try_start_2 .. :try_end_2} :catch_2

    .line 463
    :cond_4
    add-int/lit8 v3, v3, 0x1

    goto :goto_4

    .line 432
    .end local v1           #bluetoothOff:Z
    .end local v3           #i:I
    .end local v5           #mphone:Lcom/android/internal/telephony/msim/ITelephonyMSim;
    .end local v7           #nfcOff:Z
    .end local v9           #radioOff:Z
    :cond_5
    const/4 v7, 0x0

    goto :goto_0

    .line 438
    :catch_0
    move-exception v2

    .line 439
    .local v2, ex:Landroid/os/RemoteException;
    const-string v11, "ShutdownThread"

    const-string v12, "RemoteException during NFC shutdown"

    invoke-static {v11, v12, v2}, Landroid/util/Log;->e(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)I

    .line 440
    const/4 v7, 0x1

    .restart local v7       #nfcOff:Z
    goto :goto_1

    .line 444
    .end local v2           #ex:Landroid/os/RemoteException;
    :cond_6
    const/4 v1, 0x0

    goto :goto_2

    .line 449
    :catch_1
    move-exception v2

    .line 450
    .restart local v2       #ex:Landroid/os/RemoteException;
    const-string v11, "ShutdownThread"

    const-string v12, "RemoteException during bluetooth shutdown"

    invoke-static {v11, v12, v2}, Landroid/util/Log;->e(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)I

    .line 451
    const/4 v1, 0x1

    .restart local v1       #bluetoothOff:Z
    goto :goto_3

    .line 464
    .end local v2           #ex:Landroid/os/RemoteException;
    .restart local v3       #i:I
    .restart local v5       #mphone:Lcom/android/internal/telephony/msim/ITelephonyMSim;
    .restart local v9       #radioOff:Z
    :cond_7
    const/4 v9, 0x0

    goto :goto_5

    .line 472
    .end local v3           #i:I
    .end local v5           #mphone:Lcom/android/internal/telephony/msim/ITelephonyMSim;
    :cond_8
    if-eqz v8, :cond_9

    :try_start_3
    invoke-interface {v8}, Lcom/android/internal/telephony/ITelephony;->isRadioOn()Z

    move-result v11

    if-nez v11, :cond_c

    :cond_9
    const/4 v9, 0x1

    .line 473
    :goto_6
    if-nez v9, :cond_a

    .line 474
    const-string v11, "ShutdownThread"

    const-string v12, "Turning off radio..."

    invoke-static {v11, v12}, Landroid/util/Log;->w(Ljava/lang/String;Ljava/lang/String;)I

    .line 475
    const/4 v11, 0x0

    invoke-interface {v8, v11}, Lcom/android/internal/telephony/ITelephony;->setRadio(Z)Z
    :try_end_3
    .catch Landroid/os/RemoteException; {:try_start_3 .. :try_end_3} :catch_2

    .line 483
    :cond_a
    :goto_7
    const-string v11, "ShutdownThread"

    const-string v12, "Waiting for NFC, Bluetooth and Radio..."

    invoke-static {v11, v12}, Landroid/util/Log;->i(Ljava/lang/String;Ljava/lang/String;)I

    .line 485
    :goto_8
    invoke-static {}, Landroid/os/SystemClock;->elapsedRealtime()J

    move-result-wide v11

    iget-wide v13, p0, Lcom/android/server/power/ShutdownThread$5;->val$endTime:J

    cmp-long v11, v11, v13

    if-gez v11, :cond_13

    .line 486
    if-nez v1, :cond_b

    .line 488
    :try_start_4
    invoke-interface {v0}, Landroid/bluetooth/IBluetoothManager;->isEnabled()Z
    :try_end_4
    .catch Landroid/os/RemoteException; {:try_start_4 .. :try_end_4} :catch_3

    move-result v11

    if-nez v11, :cond_d

    const/4 v1, 0x1

    .line 493
    :goto_9
    if-eqz v1, :cond_b

    .line 494
    const-string v11, "ShutdownThread"

    const-string v12, "Bluetooth turned off."

    invoke-static {v11, v12}, Landroid/util/Log;->i(Ljava/lang/String;Ljava/lang/String;)I

    .line 497
    :cond_b
    if-nez v9, :cond_10

    .line 499
    const/4 v10, 0x1

    .line 500
    .local v10, subRadioOff:Z
    :try_start_5
    invoke-static {}, Landroid/telephony/MSimTelephonyManager;->getDefault()Landroid/telephony/MSimTelephonyManager;

    move-result-object v11

    invoke-virtual {v11}, Landroid/telephony/MSimTelephonyManager;->isMultiSimEnabled()Z

    move-result v11

    if-eqz v11, :cond_14

    .line 501
    const-string v11, "phone_msim"

    invoke-static {v11}, Landroid/os/ServiceManager;->checkService(Ljava/lang/String;)Landroid/os/IBinder;

    move-result-object v11

    invoke-static {v11}, Lcom/android/internal/telephony/msim/ITelephonyMSim$Stub;->asInterface(Landroid/os/IBinder;)Lcom/android/internal/telephony/msim/ITelephonyMSim;

    move-result-object v5

    .line 503
    .restart local v5       #mphone:Lcom/android/internal/telephony/msim/ITelephonyMSim;
    const/4 v3, 0x0

    .restart local v3       #i:I
    :goto_a
    invoke-static {}, Landroid/telephony/MSimTelephonyManager;->getDefault()Landroid/telephony/MSimTelephonyManager;

    move-result-object v11

    invoke-virtual {v11}, Landroid/telephony/MSimTelephonyManager;->getPhoneCount()I

    move-result v11

    if-ge v3, v11, :cond_f

    .line 505
    if-eqz v10, :cond_e

    invoke-interface {v5, v3}, Lcom/android/internal/telephony/msim/ITelephonyMSim;->isRadioOn(I)Z
    :try_end_5
    .catch Landroid/os/RemoteException; {:try_start_5 .. :try_end_5} :catch_4

    move-result v11

    if-nez v11, :cond_e

    const/4 v10, 0x1

    .line 504
    :goto_b
    add-int/lit8 v3, v3, 0x1

    goto :goto_a

    .line 472
    .end local v3           #i:I
    .end local v5           #mphone:Lcom/android/internal/telephony/msim/ITelephonyMSim;
    .end local v10           #subRadioOff:Z
    :cond_c
    const/4 v9, 0x0

    goto :goto_6

    .line 478
    :catch_2
    move-exception v2

    .line 479
    .restart local v2       #ex:Landroid/os/RemoteException;
    const-string v11, "ShutdownThread"

    const-string v12, "RemoteException during radio shutdown"

    invoke-static {v11, v12, v2}, Landroid/util/Log;->e(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)I

    .line 480
    const/4 v9, 0x1

    goto :goto_7

    .line 488
    .end local v2           #ex:Landroid/os/RemoteException;
    :cond_d
    const/4 v1, 0x0

    goto :goto_9

    .line 489
    :catch_3
    move-exception v2

    .line 490
    .restart local v2       #ex:Landroid/os/RemoteException;
    const-string v11, "ShutdownThread"

    const-string v12, "RemoteException during bluetooth shutdown"

    invoke-static {v11, v12, v2}, Landroid/util/Log;->e(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)I

    .line 491
    const/4 v1, 0x1

    goto :goto_9

    .line 505
    .end local v2           #ex:Landroid/os/RemoteException;
    .restart local v3       #i:I
    .restart local v5       #mphone:Lcom/android/internal/telephony/msim/ITelephonyMSim;
    .restart local v10       #subRadioOff:Z
    :cond_e
    const/4 v10, 0x0

    goto :goto_b

    .line 507
    :cond_f
    move v9, v10

    .line 515
    .end local v3           #i:I
    .end local v5           #mphone:Lcom/android/internal/telephony/msim/ITelephonyMSim;
    :goto_c
    if-eqz v9, :cond_10

    .line 516
    const-string v11, "ShutdownThread"

    const-string v12, "Radio turned off."

    invoke-static {v11, v12}, Landroid/util/Log;->i(Ljava/lang/String;Ljava/lang/String;)I

    .line 520
    .end local v10           #subRadioOff:Z
    :cond_10
    move v4, v9

    .line 521
    .local v4, isRealModemOff:Z
    if-nez v9, :cond_11

    invoke-static {}, Landroid/os/SystemClock;->elapsedRealtime()J

    move-result-wide v11

    iget-wide v13, p0, Lcom/android/server/power/ShutdownThread$5;->val$modemEndTime:J

    cmp-long v11, v11, v13

    if-ltz v11, :cond_11

    .line 522
    const/4 v9, 0x1

    .line 523
    const-string v11, "ShutdownThread"

    const-string v12, "Radio has not be turned off in 3000ms, now we will shutdown mobile if necessary ~"

    invoke-static {v11, v12}, Landroid/util/Log;->i(Ljava/lang/String;Ljava/lang/String;)I

    .line 527
    :cond_11
    if-nez v7, :cond_12

    .line 529
    :try_start_6
    invoke-interface {v6}, Landroid/nfc/INfcAdapter;->getState()I
    :try_end_6
    .catch Landroid/os/RemoteException; {:try_start_6 .. :try_end_6} :catch_5

    move-result v11

    const/4 v12, 0x1

    if-ne v11, v12, :cond_16

    const/4 v7, 0x1

    .line 534
    :goto_d
    if-eqz v7, :cond_12

    .line 535
    const-string v11, "ShutdownThread"

    const-string v12, "NFC turned off."

    invoke-static {v11, v12}, Landroid/util/Log;->i(Ljava/lang/String;Ljava/lang/String;)I

    .line 539
    :cond_12
    if-eqz v9, :cond_17

    if-eqz v1, :cond_17

    if-eqz v7, :cond_17

    .line 540
    const-string v11, "ShutdownThread"

    const-string v12, "NFC, Radio and Bluetooth shutdown complete."

    invoke-static {v11, v12}, Landroid/util/Log;->i(Ljava/lang/String;Ljava/lang/String;)I

    .line 541
    iget-object v11, p0, Lcom/android/server/power/ShutdownThread$5;->val$done:[Z

    const/4 v12, 0x0

    const/4 v13, 0x1

    aput-boolean v13, v11, v12

    .line 549
    .end local v4           #isRealModemOff:Z
    :cond_13
    return-void

    .line 509
    .restart local v10       #subRadioOff:Z
    :cond_14
    :try_start_7
    invoke-interface {v8}, Lcom/android/internal/telephony/ITelephony;->isRadioOn()Z
    :try_end_7
    .catch Landroid/os/RemoteException; {:try_start_7 .. :try_end_7} :catch_4

    move-result v11

    if-nez v11, :cond_15

    const/4 v9, 0x1

    :goto_e
    goto :goto_c

    :cond_15
    const/4 v9, 0x0

    goto :goto_e

    .line 511
    :catch_4
    move-exception v2

    .line 512
    .restart local v2       #ex:Landroid/os/RemoteException;
    const-string v11, "ShutdownThread"

    const-string v12, "RemoteException during radio shutdown"

    invoke-static {v11, v12, v2}, Landroid/util/Log;->e(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)I

    .line 513
    const/4 v9, 0x1

    goto :goto_c

    .line 529
    .end local v2           #ex:Landroid/os/RemoteException;
    .end local v10           #subRadioOff:Z
    .restart local v4       #isRealModemOff:Z
    :cond_16
    const/4 v7, 0x0

    goto :goto_d

    .line 530
    :catch_5
    move-exception v2

    .line 531
    .restart local v2       #ex:Landroid/os/RemoteException;
    const-string v11, "ShutdownThread"

    const-string v12, "RemoteException during NFC shutdown"

    invoke-static {v11, v12, v2}, Landroid/util/Log;->e(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)I

    .line 532
    const/4 v7, 0x1

    goto :goto_d

    .line 545
    .end local v2           #ex:Landroid/os/RemoteException;
    :cond_17
    move v9, v4

    .line 547
    const-wide/16 v11, 0x1f4

    invoke-static {v11, v12}, Landroid/os/SystemClock;->sleep(J)V

    goto/16 :goto_8
.end method
